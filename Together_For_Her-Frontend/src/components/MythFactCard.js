...
const MythFactCard = ({ type, title, content }) => {
  ...
  return (
    <div className="group relative w-full max-w-sm">
      <div className={`absolute -inset-1 bg-gradient-to-r ${isMyth ? 'from-rose-500 to-red-600' : 'from-emerald-500 to-teal-600'} rounded-[3rem] blur opacity-5 group-hover:opacity-10 transition duration-500`}></div>
      <div className="relative bg-white/40 dark:bg-slate-800/40 backdrop-blur-xl p-8 rounded-[3rem] border border-pink-200/50 dark:border-slate-500/20 shadow-xl space-y-6 hover:-translate-y-1 transition-all duration-500 h-full flex flex-col">
        <div className="flex items-center justify-between">
          ...
          <div className="w-8 h-8 rounded-full border border-slate-100 dark:border-slate-500/50 flex items-center justify-center">
